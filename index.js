...
document.addEventListener("keydown", function(evt){
    switch (evt.keyCode) {
        ...
        case 38:
            ...
            break;
        case 39:
            if (direction.current != direction.left && direction.current != direction.right) {direction.current = direction.right}
            break;
        case 40:
            if (direction.current != direction.up && direction.current != direction.down) {direction.current = direction.down}
            break;
    }
});
...
const snake = {
    ...
    update : function() {
        if (frames % 6 == 0) {
            ...
            if(this.position[0].y < 0 ) this.position[0].y = cvs.width - 10;
            if(this.position[0].y > cvs.height ) this.position[0].y = 10;

            for (let i = this.position.length - 1; i > 0; i--) {
                if (this.position[0].x == this.position[i].x && this.position[0].y == this.position[i].y  && this.position.length > 3) {
